...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        private void cashierOrderForm_addBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Silakan pilih produk terlebih dahulu", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (connect.State == ConnectionState.Closed)
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=DESKTOP-M26C3G9;Initial Catalog=aplikasiCafe;Integrated Security=True"))
            {
                connect.Open();
        ...
        {
            // Pastikan ada order dan amount diisi
            if (cashierOrderForm_amount.Text == "" || cashierOrderForm_orderTable.Rows.Count <= 0)
            {
                MessageBox.Show("Silakan masukkan jumlah dan pastikan ada pesanan.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Parsing amount yang dibayar
            if (!float.TryParse(cashierOrderForm_amount.Text, out float amountPaid))
            {
                MessageBox.Show("Bayar yang dimasukkan tidak valid.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (change < 0)
            {
                MessageBox.Show("Bayarnya kurang dari harga total!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (MessageBox.Show("Apakah Anda yakin untuk membayar?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        IDGenerator(); // Generate customer ID
                        ...
                        MessageBox.Show("Berhasil Bayar!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "Solune Cafe Shop";
            ...
            e.Graphics.DrawString("Total Harga: \tRp" + totalPrice + "\nBayar: \t\tRp"
                + cashierOrderForm_amount.Text + "\n\t\t------------\nKembali: \tRp" + cashierOrderForm_change.Text, labelFont, Brushes.Black, labelX, y);
        ...
        private void cashierOrderForm_removeBtn_Click(object sender, EventArgs e)
        {
            if (getOrderID == 0)
            {
                MessageBox.Show("Pilih barang terlebih dahulu", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (MessageBox.Show("Apakah anda yakin untuk menghapus Order ID: " + getOrderID + "?", "Confirmation Message"
                    ...
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            using(SqlCommand cmd = new SqlCommand(deleteData, connect))
                            {
                                ...
                                MessageBox.Show("Berhasil Hapus!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        catch(Exception ex)
                        {
