...
namespace CafeShopManagementSystem
{
    public partial class AdminAddProducts : UserControl
    {
                ...
                if(connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand selectPID = new SqlCommand(selectProdID, connect))
                        {
                            ...
                            {
                                MessageBox.Show("Product ID: " + adminAddProducts_id.Text.Trim() + " sudah diambil!", "Error Message"
                            ...
                            else
                            {
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertData, connect))
                                {
                                    ...
                                    MessageBox.Show("Berhasil Tambah!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    displayData();
                                }
                            }
                        }
        ...
        private void adminAddProducts_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            // Validasi konversi numeric
            if (!int.TryParse(adminAddProducts_stock.Text.Trim(), out int stock))
            {
                MessageBox.Show("Stock harus berupa angka.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!decimal.TryParse(adminAddProducts_price.Text.Trim(), out decimal price))
            {
                MessageBox.Show("Price harus berupa angka.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            DialogResult check = MessageBox.Show(
                "Apakah anda yakin untuk memperbarui Product ID: " + adminAddProducts_id.Text.Trim() + "?",
            ...
            try
            {
                ...
                // Salin file gambar baru jika ada
                string path = null;
                if (!string.IsNullOrEmpty(adminAddProducts_imageView.ImageLocation))
                {
                    path = Path.Combine(@"C:\Users\USER\source\repos\CafeShopManagementSystem\CafeShopManagementSystem\Product_Directory\"
                        + adminAddProducts_id.Text.Trim() + ".jpg");
            ...
            WHERE prod_id = @prodID";
                ...
                using (SqlCommand updateD = new SqlCommand(updateData, connect))
                {
                    ...
                    updateD.Parameters.AddWithValue("@dateUpdate", DateTime.Today);
                    updateD.Parameters.AddWithValue("@prodID", adminAddProducts_id.Text.Trim());
                    updateD.Parameters.AddWithValue("@prodImage", path ?? (object)DBNull.Value);

                    updateD.ExecuteNonQuery();
                }

                clearFields();
                MessageBox.Show("Berhasil Perbarui!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                displayData();
            }
            catch (Exception ex)
            {
        ...
        private void adminAddProducts_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Apakah anda yakin menghapus Product ID: " + adminAddProducts_id.Text.Trim() + "?"
